...
const TableAnggota = () => {
  ...
  const [input, setInput] = useState<TAnggota>({
    idAnggota: '',
    nis: '',
    nama: '',
    kelas: '',
  ...
  const handleDelete = async (packageItem: TAnggota) => {
    try {
      ...
      const response = await axios.delete(`/api/anggota/delete`, {
        headers: { 'Content-Type': 'application/json' },
        data: {
          idAnggota: packageItem?.idAnggota,
        },
      })

      anggotaState.deleteData(packageItem?.idAnggota)
      // pengembalianState.deleteData(packageItem?.idAnggota as string)

      // useInitStates({ anggotaState, anggotaState, pengembalianState, statisticState })
      // hitungDenda(anggotaState, statisticState)
  ...
  const handleSubmit = async () => {
    if (isEdit === 'create') {
      try {
        ...
        // useInitStates({ anggotaState, pengembalianState, statisticState })
        // hitungDenda(anggotaState, statisticState)
    ...
    if (isEdit === 'edit') {
      try {
        ...
        const response = await axios.post('/api/anggota/update', {
          ...
          data: { ...input, idAnggota: undefined },
        })

        // update state
        const updatedData: TAnggota[] = anggotaState.data.map((anggota) =>
    ...
    <div className="rounded-sm border border-stroke bg-white px-5 pt-6 pb-2.5 shadow-default dark:border-strokedark dark:bg-boxdark sm:px-7.5 xl:pb-1">
      <div className="max-w-full overflow-x-auto  ">
        ...
        <table className="w-full table-auto">
          ...
          <tbody>
            {anggotaState.data.map((anggotaItem, key) => {
              return (
                <tr key={key}>
                  ...
                  <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                    <p className="text-black dark:text-white">
                      {anggotaItem.kelas + ' ' + anggotaItem.kategoriKelas}
                    </p>
                  </td>
                  <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                    <p className="text-black dark:text-white">
                  ...
                  <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                    ...
                    >
                      ...
                      <Dropdown.Item
                        onClick={() => handleDelete(anggotaItem)}
                        className="flex w-full items-center gap-2 rounded-sm py-1.5 px-4 text-left text-sm hover:bg-gray dark:hover:bg-meta-4"
                      >
                        <svg
                          className="fill-current"
                ...
                <div>
                  ...
                  <CostumeDropdown
                    trigger={
                      <input
                        readOnly
                        value={new Date(input.tahunMasuk).getFullYear()}
                        type="text"
                        placeholder="Jurusan"
                        className="w-full rounded-lg border-[1.5px] border-stroke bg-transparent py-3 px-5 font-medium outline-none transition focus:border-primary active:border-primary disabled:cursor-default disabled:bg-whiter dark:border-form-strokedark dark:bg-form-input dark:focus:border-primary"
                      />
                  ...
                  >
                    {Array.from(
                      { length: new Date().getFullYear() - 1975 },
                      (_, index) => {
                        const tahun = new Date().getFullYear() - index
                        return (
                          <Button
                            key={index}
                            className="w-full"
